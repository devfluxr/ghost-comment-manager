...
class GhostPublisher
{
    ...
    public function maybe_mark_ghost(int $comment_id, array $commentdata): void
    {
    ...
    public function add_confirm_action(array $actions, \WP_Comment $comment): array
    {
        ...
        $url = wp_nonce_url(
            add_query_arg(
                ['action' => 'gcmgr_confirm_ghost', 'comment_id' => $comment->comment_ID, 'ref' => 'edit-comments.php'],
    ...
    public function confirm_ghost(): void
    {
        ...
        check_admin_referer('gcmgr_confirm_' . $comment_id);
        if ($comment_id <= 0) {
            wp_die(esc_html__('Invalid comment.', 'ghost-comment-manager'));
        }
        delete_comment_meta($comment_id, self::META_GHOST);
        Metrics::bump('ghost_confirmed');

        // 0.C — Fire after confirm.
        ...
        $redirect = add_query_arg(
            ['gcmgr_' => 'ghost_confirmed', 'cid' => $comment_id, 'gcmgr_n' => wp_create_nonce('gcmgr_notice')],
            admin_url($ref)
        );
